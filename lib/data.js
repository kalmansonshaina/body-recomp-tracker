...
// Weeks run Sunday → Saturday (Sunday is day 1), so weekend workouts stay in
// the same week as the Monday that follows them.
export const startOfWeek = (s) => { const d = parse(s); const day = d.getDay(); d.setDate(d.getDate() - day); return iso(d); };
...
export const WARMUPS = {
  ...
  full: { title: 'Full-Body Warm-Up', time: '5–7 min', steps: [
    { name: 'World’s Greatest Stretch', detail: '5 reps/side · open through hips & upper back' },
    { name: 'Banded Glute Bridges', detail: '15 reps · 2-sec squeeze at the top' },
    { name: 'Band Pull-Aparts', detail: '15 reps · shoulders down' },
    { name: 'Bodyweight Squats', detail: '10 reps · slow, full depth' },
  ], note: 'Then do 1–2 lighter warm-up sets of your first compound lift.' },
  core: { title: 'Core Warm-Up', time: '3–4 min', steps: [
    { name: 'Cat-Cow', detail: '8 reps' },
    { name: 'Dead Bug', detail: '6 reps/side · ribs down, low back flat' },
    { name: 'Bird Dog', detail: '8 reps/side · slow and stable' },
  ], note: 'Brace on every rep — quality over speed.' },
  cardio: { title: 'Cardio Warm-Up', time: '3–5 min', steps: [
    { name: 'Easy-Pace Ramp', detail: '2–3 min gradually raising pace/incline' },
    { name: 'Leg Swings', detail: '10/side front-to-back & side-to-side' },
    { name: 'Ankle Circles', detail: '10 each direction' },
  ], note: 'Start easy and build — don’t go hard cold.' },
};
export const COOLDOWNS = {
  ...
  full: { title: 'Full-Body Cooldown', time: '5 min', steps: [
    { name: 'Hip Flexor Stretch', detail: '30–45 sec/side · pelvis tucked' },
    { name: 'Figure-4 Glute Stretch', detail: '30 sec/side' },
    { name: 'Child’s Pose', detail: '45 sec · breathe into the back' },
  ] },
  core: { title: 'Core Cooldown', time: 'optional', steps: [
    { name: 'Cobra / Sphinx', detail: '20–30 sec · gentle extension' },
    { name: 'Child’s Pose', detail: '30 sec' },
  ] },
  cardio: { title: 'Cardio Cooldown', time: '3–5 min', steps: [
    { name: 'Slow Walk', detail: '2–3 min bringing your heart rate down' },
    { name: 'Calf Stretch', detail: '30 sec/side' },
    { name: 'Quad Stretch', detail: '30 sec/side' },
  ] },
};
...
// Build a fresh editable workout for a given day (defaults to today), seeding
// each exercise's weight from the last time it was performed.
export function makeActive(S, type, date = today()) {
  const w = workoutById(S, type);
  return {
    id: uid(), date, type, name: w.name, group: w.group || 'lower',
...
/* ---- fitness knowledge: movement patterns, compounds, finishers --------- */
// Infer a movement pattern from an exercise's name + muscles. Order matters —
// more specific rules first (e.g. leg curl → hamstring before any "curl").
const PAT_RULES = [
  ['hinge', /deadlift|rdl|hip thrust|glute bridge|good ?morning|back ?ext|hyperext|pull-?through|kettlebell swing|kb swing|hip extension/i],
  ['glute', /glute kickback|cable kickback|donkey kick|hip abduction|abductor|abduction|clamshell|monster walk|lateral walk|frog pump|glute med|fire hydrant/i],
  ['squat', /squat|leg press|hack|sissy|wall sit|pendulum/i],
  ['lunge', /lunge|split squat|step-?up|bulgarian|curtsy/i],
  ['hamstring', /leg curl|hamstring|nordic|lying curl|seated leg curl/i],
  ['quad', /leg extension|quad/i],
  ['calf', /calf|heel raise/i],
  ['verticalPull', /pull-?up|pull-?down|lat pull|chin-?up/i],
  ['horizontalPull', /\brow\b|face pull|rear delt|reverse fly|rear.?delt|pullover|seal row/i],
  ['verticalPush', /overhead|shoulder press|military|arnold|lateral raise|front raise|push press|\bdelt\b/i],
  ['triceps', /tricep|pushdown|skull ?crusher|overhead extension|close-?grip|kickback.*tri/i],
  ['biceps', /bicep|hammer curl|preacher|concentration curl|\bcurl\b/i],
  ['core', /plank|crunch|sit-?up|dead ?bug|pallof|russian twist|leg raise|hollow|ab wheel|woodchop|mountain climber|bird ?dog|toes to bar|hanging|v-?up|bicycle/i],
  ['horizontalPush', /bench|chest|incline|decline|push-?up|\bdip\b|pec|\bfly\b|floor press|\bpress\b/i],
];
export function movePattern(e) {
  const s = `${e.name} ${e.muscles || ''}`;
  for (const [pat, re] of PAT_RULES) if (re.test(s)) return pat;
  return e.group === 'core' ? 'core' : e.group === 'upper' ? 'horizontalPull' : 'squat';
}
const COMPOUND_PATTERNS = new Set(['hinge', 'squat', 'lunge', 'horizontalPush', 'verticalPush', 'horizontalPull', 'verticalPull']);
const ISO_PATTERNS = new Set(['glute', 'quad', 'hamstring', 'calf', 'biceps', 'triceps', 'core']);
export const isCompoundEx = (e) => /compound/i.test(e.cat || '') || COMPOUND_PATTERNS.has(movePattern(e));
// Pattern order per focus — women's recomp: posterior chain & glutes first.
const FOCUS_SEQ = {
  full: ['hinge', 'squat', 'horizontalPush', 'horizontalPull', 'lunge', 'glute', 'core', 'verticalPush'],
  lower: ['hinge', 'squat', 'glute', 'lunge', 'hamstring', 'glute', 'quad', 'calf'],
  upper: ['horizontalPush', 'verticalPull', 'horizontalPull', 'verticalPush', 'biceps', 'triceps', 'horizontalPush', 'core'],
  core: ['core', 'core', 'core', 'core', 'core', 'core'],
};
export const FINISHERS = {
  full: { name: 'Full-body metabolic', detail: 'KB swing ×20 → goblet squat ×15 → push-up ×10 → mountain climbers 30s. 3 rounds, ~30s rest.' },
  lower: { name: 'Glute burnout', detail: 'Banded hip thrust ×25 → banded lateral walk ×20/side → glute-bridge hold 30s. 2–3 rounds, minimal rest.' },
  upper: { name: 'Upper pump', detail: 'Push-ups to near-failure → band pull-aparts ×25 → curls ×20. 2 rounds.' },
  core: { name: 'Core circuit', detail: 'Hollow hold 30s → reverse crunch ×15 → plank 45s → Russian twist ×20. 2–3 rounds.' },
};

// On-device workout generator — builds a balanced session for a chosen focus,
// time and style using movement-pattern knowledge, favouring compound lifts
// and movements you've done before (so weights pre-fill), glute-forward.
export const GEN_FOCUS = [
  ['full', 'Full body'], ['lower', 'Lower / Glutes'], ['upper', 'Upper body'], ['core', 'Core / Abs'],
];
export const GEN_STYLE = [
  ['balanced', 'Balanced'], ['strength', 'Strength'], ['finisher', 'Quick finisher'],
];
export const GEN_MINUTES = [15, 30, 45, 60];
export function generateWorkout(S, { focus = 'full', minutes = 30, style = 'balanced' } = {}) {
  const pool = exList(S).filter((e) => ['lower', 'upper', 'core'].includes(e.group));
  const hasHist = (k) => !!lastPerf(S, k) || !!(S.exState && S.exState[k]);
  const per = style === 'strength' ? 7 : style === 'finisher' ? 4 : 5.5;
  const count = clamp(Math.round(minutes / per), 3, 8);
  const seq = FOCUS_SEQ[focus] || FOCUS_SEQ.full;
  const patterns = Array.from({ length: count }, (_, i) => seq[i % seq.length]);
  const exclude = new Set();
  const groupOf = (pat) => (pat === 'core' ? 'core' : ['horizontalPush', 'verticalPush', 'horizontalPull', 'verticalPull', 'biceps', 'triceps'].includes(pat) ? 'upper' : 'lower');
  const pickForPattern = (pat) => {
    let cands = pool.filter((e) => movePattern(e) === pat && !exclude.has(e.key));
    if (!cands.length) cands = pool.filter((e) => e.group === groupOf(pat) && !exclude.has(e.key));
    if (!cands.length) cands = pool.filter((e) => !exclude.has(e.key));
    if (!cands.length) return null;
    // Bias slightly toward movements you've done (so weights pre-fill) but pick
    // at random so every shuffle meaningfully changes the exercises.
    const hist = cands.filter((e) => hasHist(e.key));
    const bag = (hist.length && Math.random() < 0.4) ? hist : cands;
    const pick = bag[Math.floor(Math.random() * bag.length)];
    exclude.add(pick.key); return pick;
  };
  const chosen = patterns.map(pickForPattern).filter(Boolean);
  const base = style === 'strength' ? { sets: 4, min: 5, max: 8 }
    : style === 'finisher' ? { sets: 3, min: 15, max: 20 }
      : { sets: 3, min: 8, max: 12 };
  const exercises = chosen.map((e) => {
    const iso = ISO_PATTERNS.has(movePattern(e));
    const min = iso ? Math.max(base.min + 4, 12) : base.min;
    const max = iso ? Math.min(base.max + (style === 'strength' ? 7 : 5), 20) : base.max;
    const last = lastPerf(S, e.key);
    const prevW = last ? Math.max(...last.ex.sets.filter((s) => s.reps).map((s) => +s.weight || 0)) : (S.exState && S.exState[e.key] ? S.exState[e.key].weight : '');
    return { key: e.key, min, max, perSide: false, prescribed: base.sets,
      sets: Array.from({ length: base.sets }, () => ({ weight: prevW || '', reps: '' })), rir: 2, pain: 'none', equip: {}, note: '' };
  });
  const focusLabel = { full: 'Full Body', lower: 'Lower / Glutes', upper: 'Upper Body', core: 'Core' }[focus] || 'Workout';
  const styleLabel = style === 'finisher' ? ' Finisher' : style === 'strength' ? ' Strength' : '';
  const rest = style === 'strength' ? 'Rest 2–3 min on compounds, ~90s on accessories.'
    : style === 'finisher' ? 'Move fast — 30–45s rest, superset where you can.'
      : 'Rest 60–90s between sets; keep 1–2 reps in reserve.';
  const fin = FINISHERS[focus] || FINISHERS.full;
  const warm = (WARMUPS[focus] || {}).title || 'Dynamic warm-up';
  const note = style === 'finisher' ? `Circuit-style — ${rest}` : `${rest}\nOptional finisher — ${fin.name}: ${fin.detail}`;
  return { id: uid(), date: today(), type: 'generated', name: `${minutes}-min ${focusLabel}${styleLabel}`,
    group: focus, exercises, cardio: null, ab: false, warmup: false, cooldown: false, note, generated: true,
    guidance: { warmup: warm, rest, finisher: style === 'finisher' ? null : fin } };
}
