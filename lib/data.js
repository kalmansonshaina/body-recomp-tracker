...
// On-device workout generator — builds a session for a chosen focus, time and
// style, favouring movements you've done before (so weights pre-fill) and
// compound lifts first, with variety via shuffling.
...
export function generateWorkout(S, { focus = 'full', minutes = 30, style = 'balanced' } = {}) {
  ...
  const isCompound = (e) => /compound/i.test(e.cat || '');
  const shuffle = (arr) => arr.map((v) => [Math.random(), v]).sort((a, b) => a[0] - b[0]).map((x) => x[1]);
  const pickFrom = (group, n, exclude) => {
    const c = pool.filter((e) => e.group === group && !exclude.has(e.key));
    const tiers = [
      shuffle(c.filter((e) => hasHist(e.key) && isCompound(e))),
      shuffle(c.filter((e) => !hasHist(e.key) && isCompound(e))),
      shuffle(c.filter((e) => hasHist(e.key) && !isCompound(e))),
      shuffle(c.filter((e) => !hasHist(e.key) && !isCompound(e))),
    ].flat();
    const out = [];
    for (const e of tiers) { if (out.length >= n) break; out.push(e); exclude.add(e.key); }
    return out;
  };
  ...
  let chosen = [];
  if (focus === 'full') {
    const nCore = count >= 4 ? 1 : 0;
    const rem = count - nCore;
    const nLower = Math.ceil(rem / 2), nUpper = rem - nLower;
    const g = { lower: pickFrom('lower', nLower, exclude), upper: pickFrom('upper', nUpper, exclude), core: pickFrom('core', nCore, exclude) };
    let added = true;
    while (added) { added = false; for (const k of ['lower', 'upper', 'core']) { if (g[k].length) { chosen.push(g[k].shift()); added = true; } } }
  } else {
    chosen = pickFrom(focus, count, exclude);
  }
  const scheme = style === 'strength' ? { sets: 4, min: 6, max: 10 }
    : style === 'finisher' ? { sets: 2, min: 12, max: 20 }
      : { sets: 3, min: 8, max: 12 };
  const exercises = chosen.map((e) => {
    ...
    return { key: e.key, min: scheme.min, max: scheme.max, perSide: false, prescribed: scheme.sets,
      sets: Array.from({ length: scheme.sets }, () => ({ weight: prevW || '', reps: '' })), rir: 2, pain: 'none', equip: {}, note: '' };
  });
  ...
  return { id: uid(), date: today(), type: 'generated', name: `${minutes}-min ${focusLabel}${styleLabel}`,
    group: focus === 'upper' ? 'upper' : 'lower', exercises, cardio: null, ab: false, warmup: false, cooldown: false, note: '', generated: true };
}
